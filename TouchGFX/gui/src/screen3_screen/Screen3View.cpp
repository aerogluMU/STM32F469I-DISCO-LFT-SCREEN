...
void Screen3View::handleTickEvent(){
	...
	if(lftflg.uart6_rx_flag == 1){
		// Clear textArea2Buffer
		memset(&textArea2Buffer,'\0',100);
		...
		if(lftflg.uart6_doors_flag == 1){
			// if 817 set, set boxDoors color GREEN
			boxDoors.setColor(touchgfx::Color::getColorFromRGB(22, 184, 46));
			...
			// if 817 is not set, set boxDoors color RED
			boxDoors.setColor(touchgfx::Color::getColorFromRGB(189, 50, 38));
		...
		// Send end line data to USART6
		char *endLine = "\r\n";
		HAL_UART_Transmit(&huart6, (uint8_t*)endLine, 2, HAL_MAX_DELAY);
